...
export default function BudgetSubitemSync({ items, loading, onRefresh }) {
  ...
  const budgetMappings = useMemo(() => {
    return budgetItems.map(budget => {
      // Find operational items that match category or department
      const matches = operationalItems.filter(op => 
        (op.budgetCategories && budget.budgetCategories && op.budgetCategories === budget.budgetCategories) ||
        (op.department && budget.department && op.department === budget.department)
      );
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3 pt-2">
                    <div className="space-y-1">
                      ...
                      <p className="text-xs font-medium">{budget.department || '—'}</p>
                    </div>
                    <div className="space-y-1">
