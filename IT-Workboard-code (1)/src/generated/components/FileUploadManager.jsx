...
export default function FileUploadManager({ item, onUploadComplete }) {
      ...
      for (let i = 0; i < totalFiles; i++) {
        ...
        try {
          await board.item(item.id).uploadFile({
            columnId: 'file_mm3z53ez', // logAnalysisReports column
            file
          }).execute();
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Existing Files */}
      {item.logAnalysisReports && item.logAnalysisReports.length > 0 && (
        <Card className="border-border/60">
          <CardHeader>
            <CardTitle className="text-sm font-semibold text-primary flex items-center gap-2">
              <FileText className="h-5 w-5" />
              Existing Log Files ({item.logAnalysisReports.length})
            </CardTitle>
            <CardDescription>
              Previously uploaded log analysis reports
            </CardDescription>
          </CardHeader>
          <CardContent>
