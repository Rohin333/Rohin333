...
import CyberpunkCard from '@generated/components/CyberpunkCard';
...
function GaugeCard({ value, label, sub, color }) {
  const hoverStyle = value >= 75 ? { boxShadow: 'var(--glow-primary-subtle)' } :
                     value < 60 ? { boxShadow: 'var(--glow-accent-faint)' } : undefined;
  
  return (
    <Card className={cn(
      "relative overflow-hidden transition-all",
      value >= 75 && "border-primary/60 hover:border-primary",
      value < 60 && "border-accent/60 hover:border-accent",
      value >= 60 && value < 75 && "border-chart-4/60 hover:border-chart-4"
    )}
    style={hoverStyle}>
      {/* Corner glow */}
      <div className={cn(
        "absolute top-0 right-0 w-12 h-12 blur-xl opacity-30",
        value >= 75 && "bg-primary",
        value < 60 && "bg-accent",
        value >= 60 && value < 75 && "bg-chart-4"
      )} />
      
      <CardContent className="p-4 flex flex-col items-center justify-center relative z-10">
        <Gauge value={value} size={100} thickness={12} startAngle={-120} endAngle={120}>
          <GaugeIndicator>
            <GaugeTrack className="stroke-muted/40" />
            <GaugeRange className={cn(
              color,
              "drop-shadow-[0_0_6px_currentColor]"
            )} />
          </GaugeIndicator>
          <GaugeValueText className="text-xl font-black text-foreground tracking-tight" />
        </Gauge>
        <p className="text-[10px] font-bold text-foreground mt-3 uppercase tracking-[0.08em]">{label}</p>
        <p className="text-[9px] text-muted-foreground uppercase tracking-wider font-medium">{sub}</p>
      </CardContent>
    </Card>
  );
}
...
function ClickableTile({ icon: Icon, label, value, sub, accent, destructive, onClick }) {
  const hoverStyle = destructive ? { boxShadow: 'var(--glow-accent-subtle)' } :
                     accent ? { boxShadow: 'var(--glow-primary-subtle)' } :
                     { boxShadow: 'var(--glow-primary-faint)' };
  
  const iconGlow = destructive ? { filter: 'drop-shadow(var(--glow-accent-strong))' } :
                   accent ? { filter: 'drop-shadow(var(--glow-primary-strong))' } : undefined;
  
  const valueGlow = destructive ? { textShadow: 'var(--glow-accent-medium)' } :
                    accent ? { textShadow: 'var(--glow-primary-medium)' } : undefined;
  
  return (
    <Card 
      className={cn(
        "relative overflow-hidden transition-all cursor-pointer group",
        destructive && "border-accent/60 hover:border-accent",
        accent && "border-primary/60 hover:border-primary",
        !destructive && !accent && "border-border/40 hover:border-border"
      )}
      onClick={onClick}
      style={hoverStyle}
    >
      ...
      <div className={cn(
        "absolute top-0 right-0 w-0 h-0 border-t-[20px] border-l-[20px] border-l-transparent",
        destructive && "border-t-accent/30",
        accent && "border-t-primary/30",
        !destructive && !accent && "border-t-border/20"
      )} />
      
      <CardContent className="p-3 flex items-center gap-3 relative z-10">
        <div className={cn(
          "h-9 w-9 rounded flex items-center justify-center flex-shrink-0 relative",
          destructive && "bg-accent/20 border border-accent/40",
          accent && "bg-primary/20 border border-primary/40",
          !destructive && !accent && "bg-muted border border-border/40"
        )}>
          <Icon className={cn(
            "h-4 w-4",
            destructive && "text-accent",
            accent && "text-primary",
            !destructive && !accent && "text-muted-foreground"
          )} 
          style={iconGlow} />
        </div>
        <div className="min-w-0 flex-1">
          <p className={cn(
            "text-xl font-black tabular-nums leading-tight tracking-tight",
            destructive && "text-accent",
            accent && "text-primary",
            !destructive && !accent && "text-foreground"
          )}
          style={valueGlow}>{value}</p>
          <p className="text-[9px] text-muted-foreground truncate uppercase tracking-wider font-semibold">{label}</p>
          {sub && <p className="text-[9px] text-muted-foreground/60 truncate uppercase tracking-wide">{sub}</p>}
        </div>
        <ExternalLink className="h-3 w-3 text-muted-foreground/20 group-hover:text-primary/80 transition-all flex-shrink-0"
