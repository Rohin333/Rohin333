...
export default function ITContractsView({ items, loading }) {
  const metrics = useMemo(() => {
    ...
    const active = contractItems.filter(c => 
      c.status === 'In Progress' || c.currentStatus === 'In Progress'
    ).length;
                ...
                >
                    ...
                    {contract.department && (
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          <p className="font-medium text-foreground truncate">{contract.department}</p>
                        </div>
                      </div>
                    )}
                  </div>

                  {contract.licensingProposalRequest && (
                    <div className="mb-3 p-3 bg-muted/30 rounded">
                      <p className="text-xs text-muted-foreground mb-1">Licensing Details</p>
                      <p className="text-sm text-foreground">{contract.licensingProposalRequest}</p>
                    </div>
                  )}
                  ...
                  {contract.summary && (
                    <div className="mb-3 p-3 bg-muted/30 rounded">
                      ...
                      <p className="text-sm text-foreground">{contract.summary}</p>
                    </div>
                  )}
