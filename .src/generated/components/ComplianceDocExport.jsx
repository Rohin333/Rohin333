...
function buildDocumentBody(theme, legalNum, t, c, r, today) {
  ...
  children.push(advancedTable({
    ...
    rows: [
      ...
      ['Current Status', 'currentStatus', 'Status', 'Infrastructure health — Completed, In Progress, Blocked, Not Started'],
  ...
  children.push(advancedTable({
    ...
    rows: [
      ...
      ['Blocked Items', 'currentStatus = "Blocked"', '× 8', String(c.blockedHealth || 0), String((c.blockedHealth || 0) * 8)],
  ...
  children.push(p('aggregate().groupBy("currentStatus").countItems("count")', { bullet: true }));
  children.push(p(''));
  ...
  children.push(advancedTable({
    ...
    rows: [
      ...
      ['6', 'Health Distribution', 'aggregate().groupBy("currentStatus")', 'Count per health state'],
