  ...
  const handleApplyCosts = async () => {
    ...
    try {
      ...
      await board.item(item.id).update({
        ...
        currency: extractedData.currency || 'BHD',
        department: extractedData.department || null,
        directorate: extractedData.directorate || null,
        budgetCategories: extractedData.budgetCategory || null,
        year: yearDate,
        monthlyProjectAmounts: extractedData.monthlyProjectAmounts || null,
        monthlyOperationalAmounts: extractedData.monthlyOperationalAmounts || null,
        totalBudgetAmounts: extractedData.totalBudgetAmount || `${extractedData.annualCost} ${extractedData.currency || 'BHD'}`,
        overview: extractedData.overview || null,
        itBudgetStatus: 'Approved',
        approved: true,
        summary: extractedData.description ? 
          `${item.summary || ''}\n\n💰 Budget: ${extractedData.description}`.trim() : 
          item.summary
      }).execute();
